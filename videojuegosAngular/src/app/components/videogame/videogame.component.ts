...
export class VideogameComponent implements OnInit, OnDestroy {
  ...
  loadSystems(): void {
    this.subscription = this.videogameService.allSystems().subscribe((rsp) => {
      if (rsp.success === true) {
  ...
  loadSupports(): void {
    this.subscription = this.videogameService.allSupports().subscribe((rsp) => {
      if (rsp.success === true) {
    ...
    if (event.target.files && event.target.files.length) {
      ...
      reader.onload = () => {
        if (event.target.name === 'coverPage') {
          ...
          this.coverImg = this.forma.controls['coverPage'].value.replace(
            /['"]+/g,
            ''
          );
        }
  ...
  createModel(): void {
    ...
    this.game.coverPage = JSON.stringify(
      this.forma.controls['coverPage'].value
    );
